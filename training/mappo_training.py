...
class MAPPOTrainer:
        ...
        for episode in range(self.num_episodes):
            ...
            while not done:
                ...
                for agent_id, agent in enumerate(self.planner.agents):
                    enriched_obs = self.enrich_obs_with_belief(obs[agent_id], agent, shared_uncertainty_coarse)
                ...
                for i, agent in enumerate(self.planner.agents):
                    ...
                    # 1) salva belief della patch prima dell'update
                    patch_coords = []
                    old_beliefs = []
                    ...
                    for dx in range(-1, 2):
                        for dy in range(-1, 2):
                            ...
                            if 0 <= nx < self.env.field_size and 0 <= ny < self.env.field_size:
                                patch_coords.append((nx, ny))
                                old_beliefs.append(agent.belief_map[nx, ny].copy())

                    # 2) parsifica osservazione
                    obs_i = next_obs[i]

                    align_start = 0
                    align_end = align_start + 9
    ...
    def compute_shared_uncertainty_map(self):
        """
        Restituisce una mappa (H, W) con incertezza media sui belief degli agenti.
        Normalizzata in [0, 1].
        """
        beliefs = np.stack(
        ...
        entropy /= np.log(self.planner.agents[0].num_classes)  # shape: (N, H, W)

        shared_uncertainty = np.mean(entropy, axis=0).astype(np.float32)  # shape: (H, W)
